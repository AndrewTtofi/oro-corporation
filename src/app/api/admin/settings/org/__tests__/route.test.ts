...
async function loadRoute(db: PrismaClient) {
  ...
  return import("@/app/api/admin/settings/org/route");
}
...
describe("admin/settings/org PATCH", () => {
  it("unauth: throws", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      ...
      await expect(
        PATCH(makeReq({ method: "PATCH", body: { displayName: "Acme" } }))
      ).rejects.toThrow();
    });
  });
  ...
  it("happy path: upserts org settings → 200", async () => {
    await inRollbackTx(prisma, async (rawTx) => {
      const tx = wrapTx(rawTx);
      const staff = await createStaff(tx);
      sessionState.user = { id: staff.id, email: staff.email, fullName: staff.fullName, role: "staff" };
      ...
      const res = await PATCH(
        makeReq({ method: "PATCH", body: { displayName: "Acme Corp", contactEmail: "info@acme.com", address: null } })
      );
      expect(res.status).toBe(200);
